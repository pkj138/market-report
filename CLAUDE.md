...
## 0. 휴장일 체크 (가장 먼저, 무조건 실행)

리포트 생성 프로세스를 시작하기 전에 **가장 먼저** 오늘(KST, `date +%u`와 `date +%Y-%m-%d` 기준)이 한국거래소(KRX) 개장일인지 확인한다. 이 체크는 0700/1200/1800 세 회차 모두에 예외 없이 적용한다(0700은 전일 미국증시 위주 리포트이지만, 그래도 국내 휴장일에는 생성하지 않는다).

- 오늘이 토요일 또는 일요일이면 (`date +%u`가 6 또는 7) → 휴장.
- 오늘이 `holidays.json`의 해당 연도 목록에 있는 날짜이면 → 휴장.
- 위 두 조건에 모두 해당하지 않으면 → 개장일, 아래 절차대로 리포트를 정상 생성한다.
- `holidays.json`에 아직 반영되지 않은 임시공휴일(예: 선거일, 정부가 뒤늦게 발표하는 임시공휴일)이 의심되면 웹 검색으로 그날 KRX 휴장 여부를 추가로 확인한다.

**휴장일로 판단되면 그 즉시 작업을 중단한다: HTML 리포트를 생성하지 않고, git add/commit/push도 하지 않고, 어떤 산출물도 남기지 않은 채 종료한다.** "휴장이었다"는 내용을 담은 리포트조차 만들지 않는다 — 파일을 아예 만들지 않는 것이 규칙이다.
